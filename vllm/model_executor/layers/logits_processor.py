...
class LogitsProcessor(nn.Module):
    ...
    def forward(
        ...
        if self.logits_as_input:
            ...
            torch.distributed.barrier()
            # if torch.distributed.get_rank() == 0:
            print(f"myid {torch.distributed.get_rank()} hidden_states before pruning shape {hidden_states.shape} sampling_metadata type {type(sampling_metadata)} {sampling_metadata.selected_token_indices}\n", flush=True)
