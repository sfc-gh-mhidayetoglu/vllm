...
class LlamaModel(nn.Module):
    ...
    def forward(
        ...
        torch.cuda.synchronize()
        torch.distributed.barrier()
        print(f"myid {torch.distributed.get_rank()} after allgather hidden_states {type(hidden_states)} shape {hidden_states.shape}", flush=True)
...
class LlamaForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def forward(
        ...
        torch.distributed.barrier()
        if torch.distributed.get_rank() == 0:
            print(f"llama model_output {model_output.shape}")
        return model_output
    ...
    def compute_logits(
        ...
        torch.distributed.barrier()
        if torch.distributed.get_rank() == 0:
            print(f"compute_logits hidden_states {hidden_states.shape}")
